...
# Qt
WORKDIR /opt
RUN wget -q https://download.qt.io/official_releases/qt/5.15/5.15.2/single/qt-everywhere-src-5.15.2.tar.xz && \
...
RUN ./configure -opensource -confirm-license -release -static -nomake tests -nomake examples -no-compile-examples -no-icu -xcb -xcb-xlib -bundled-xcb-xinput && \
    ...
    make install
RUN cd /opt && rm -rf qt-everywhere-src-5.15.2
