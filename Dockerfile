...
# Qt
WORKDIR /opt
RUN wget -q https://download.qt.io/official_releases/qt/5.12/5.12.3/single/qt-everywhere-src-5.12.3.tar.xz
RUN tar xf qt-everywhere-src-5.12.3.tar.xz
RUN rm qt-everywhere-src-5.12.3.tar.xz
WORKDIR /opt/qt-everywhere-src-5.12.3
...
RUN make install
RUN cd /opt && rm -rf qt-everywhere-src-5.12.3

WORKDIR /

# Path
ENV PATH="${PATH}:/usr/local/Qt-5.12.3/bin"
