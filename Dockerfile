...
# Qt
WORKDIR /opt
RUN wget -q https://download.qt.io/official_releases/qt/5.14/5.14.2/single/qt-everywhere-src-5.14.2.tar.xz && \
    tar xf qt-everywhere-src-5.14.2.tar.xz && \
    rm qt-everywhere-src-5.14.2.tar.xz
WORKDIR /opt/qt-everywhere-src-5.14.2
RUN ./configure -opensource -confirm-license -release -static -nomake tests -nomake examples -no-compile-examples -no-icu -qt-xcb && \
    ...
    make install
RUN cd /opt && rm -rf qt-everywhere-src-5.14.2

WORKDIR /

# Path
ENV PATH="${PATH}:/usr/local/Qt-5.14.2/bin"
