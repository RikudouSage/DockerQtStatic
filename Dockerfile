...
# Qt
WORKDIR /opt
RUN wget -q https://download.qt.io/official_releases/qt/5.15/5.15.1/single/qt-everywhere-src-5.15.1.tar.xz && \
    tar xf qt-everywhere-src-5.15.1.tar.xz && \
    rm qt-everywhere-src-5.15.1.tar.xz
WORKDIR /opt/qt-everywhere-src-5.15.1
RUN ./configure -opensource -confirm-license -release -static -nomake tests -nomake examples -no-compile-examples -no-icu -xcb -xcb-xlib -bundled-xcb-xinput && \
    ...
    make install
RUN cd /opt && rm -rf qt-everywhere-src-5.15.1

WORKDIR /

# Path
ENV PATH="${PATH}:/usr/local/Qt-5.15.1/bin"
